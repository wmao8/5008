...
char *commands[4] = {"cd", "help", "exit", "game"};
...
void *help(void *input) {
    ...
    printf("cd ~: take back to root of directory \n");
    printf("cd _folder_: enter the folder \n");
    printf("help: list of commands and their functions\n");
    printf("game: This is a game command based on rubrics\n");
...
void *game(void *input) {
    ...
    srand(time(NULL));
    int seed = time(NULL);
    int number = rand_r(&seed) % 10 + 1;  // Random number between 1 and 10
    int guess = 0;
    int attempts = 0;
...
CommandFuncPtr command_functions[4] = {cd, help, exit_shell, game};
