...
TEST(contains, givenEmptyListThenReturnsFalse) {
    char* test = strdup("test"); //[drew] this needs to be freed
...
DestroyLinkedList(list);//[drew] added
}
...
// [drew] you either need to destroy `node` or pass in the list->head
...
free(testString); //[drew] uncommented
free(toDelete); //[drew] uncommented
DestroyLinkedList(list);//[drew] added
}
...
free(test); //[drew] uncommented
free(test2); //[drew] uncommented
DestroyLinkedList(list); //[drew] added
}
...
free(testString1); //[drew] uncommented
free(testString2); //[drew] uncommented
// free(str);
DestroyLinkedList(list); //[drew] added
DestroyLLIter(iter); //[drew] added
}
